...
Pairing is a two-layer protocol:

1. a **human-facing five-emoji pairing password/rendezvous secret**;
2. a **hidden cryptographic pairing session** that protects the long-lived relationship key.

The user interacts with only the five emojis.

The Worker may keep its existing internal invitation capability/token, expiry, failed-attempt limit, and one-time consumption. These are transport/security machinery, not user credentials.

### Emoji transport

The partner supplies the five canonical emojis to the app. The Worker resolves/rendezvouses the invitation and the client pairing session establishes the hidden key transport. The raw relationship key must never enter the Worker API.

### Share-link transport

The preferred link is:

    https://rucola.njco.dev/<five-emojis>

The path identifies the short-lived invitation. It must contain no raw relationship key, cloud credential, invitation token, or serialized pairing package.

The HTTP route is non-consuming:

- GET/HEAD must not activate or consume pairing;
- browser/link-preview fetches must be safe;
- actual invitation consumption happens through the authenticated application pairing flow;
- responses are Cache-Control: no-store;
- the route uses a restrictive referrer policy;
- pairing paths are not intentionally indexed or included in analytics;
- diagnostic/access logging must avoid recording the raw pairing path.

The Worker must decode URL UTF-8/percent-encoding exactly once and map the result to the canonical five-emoji representation before validation. URL string formatting is not itself the cryptographic representation.

### Cryptographic boundary

The first online prototype retains a locally generated random 256-bit relationship encryption key.
...
The hidden client pairing session currently uses a CPace draft-20 prototype via `@cipherman/pake-js@0.1.1`, pinned by the explicit `rucola-cpace20-v1` protocol version. This is not yet production-approved: the Expo/Hermes runtime, upstream vectors, dependency provenance, and independent security review remain validation gates. Five emojis are low entropy and therefore must not be treated as a long-lived encryption key or used to protect a server-readable copy of the relationship key.
