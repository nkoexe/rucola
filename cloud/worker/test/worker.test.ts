...
describe("Rucola cloud worker", () => {
  it("reports a healthy migrated database", async () => {
    ...
    await expect(response.json()).resolves.toMatchObject({
      ...
      version: "sync-hardening-1",
    });
  });

  it("exposes all six C2 tables after migrations", async () => {
    ...
    expect(body.tables).toEqual([
      ...
      { table: "message_receipts", present: true },
    ]);
  });
  ...
  it("authenticates a non-revoked device using only the credential hash", async () => {
    ...
    const now = Date.now();
    await env.DB.prepare(`INSERT INTO relationships (id, status, next_server_seq, created_at) VALUES (?1, 'ACTIVE', 1, ?2)`).bind("relationship-test", now).run();
    await env.DB.prepare(`INSERT INTO devices (id, relationship_id, participant, credential_hash, created_at, last_seen_at) VALUES (?1, ?2, 'ME', ?3, ?4, ?4)`).bind("device-test", "relationship-test", credentialHash, now).run();
    const response = await exports.default.fetch("https://rucola.test/v1/auth/probe", { headers: { authorization: `Bearer ${token}` } });
    expect(response.status).toBe(200);
  ...
  it("bootstraps a pairing atomically with a first device and invitation", async () => {
    ...
    const device = await env.DB.prepare(`SELECT participant, credential_hash FROM devices WHERE id = ?1`).bind(body.deviceId).first<{ participant: string; credential_hash: string }>();
  ...
  it("accepts a valid invitation exactly once", async () => {
    ...
    const response = await exports.default.fetch("https://rucola.test/v1/pairing/accept", {
      method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ token: body.token, confirmationCode: body.confirmationCode }),
    });
    expect(response.status).toBe(201);
    ...
    const devices = await env.DB.prepare(`SELECT participant, revoked_at FROM devices WHERE relationship_id = ?1 ORDER BY participant`).bind(body.relationshipId).all<{ participant: string; revoked_at: number | null }>();
    ...
    const invitation = await env.DB.prepare(`SELECT consumed_at, consumed_by_device_id FROM invitations WHERE id = ?1`).bind(body.invitationId).first<{ consumed_at: number | null; consumed_by_device_id: string | null }>();
  ...
  it("rejects an invalid confirmation code without mutating pairing state", async () => {
    const body = await bootstrap();
    const response = await exports.default.fetch("https://rucola.test/v1/pairing/accept", {
      method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ token: body.token, confirmationCode: "000000" }),
    });
    expect(response.status).toBe(400);
    const devices = await env.DB.prepare(`SELECT COUNT(*) AS count FROM devices WHERE relationship_id = ?1`).bind(body.relationshipId).first<{ count: number }>();
    expect(devices?.count).toBe(1);
  });

  it("rejects an expired invitation without creating the second device", async () => {
    const body = await bootstrap();
    await env.DB.prepare(`UPDATE invitations SET expires_at = ?1 WHERE id = ?2`).bind(Date.now() - 1, body.invitationId).run();
    const response = await exports.default.fetch("https://rucola.test/v1/pairing/accept", {
      method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ token: body.token, confirmationCode: body.confirmationCode }),
    });
    expect(response.status).toBe(400);
  ...
  it("allows a pairing invitation to be regenerated by the first authenticated device", async () => {
    ...
    const response = await exports.default.fetch("https://rucola.test/v1/pairing/create", {
      method: "POST", headers: { authorization: `Bearer ${body.credential}`, "content-type": "application/json" }, body: JSON.stringify({ expiresInSeconds: 120 }),
    });
    expect(response.status).toBe(201);
  ...
  it("lets only one of many concurrent accepts consume an invitation", async () => {
    const body = await bootstrap();
    const requests = Array.from({ length: 10 }, () => exports.default.fetch("https://rucola.test/v1/pairing/accept", {
      method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ token: body.token, confirmationCode: body.confirmationCode }),
    }));
  ...
  it("does not trust a client-supplied relationship or participant", async () => {
    const body = await bootstrap();
    const response = await exports.default.fetch("https://rucola.test/v1/auth/probe", { headers: { authorization: `Bearer ${body.credential}` } });
    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toEqual({ authenticated: true, participant: "ME" });
  });
