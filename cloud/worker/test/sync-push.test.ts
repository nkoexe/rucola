...
async function bootstrapAndAccept(): Promise<{ me: BootstrapBody; partner: AcceptedBody }> {
  ...
  const partner = (await json(acceptResponse)) as unknown as AcceptedBody;
  return { me, partner };
}
...
describe("Rucola mailbox push", () => {
  it("accepts a first message and allocates serverSeq exactly once", async () => {
    ...
    const response = await push(me.credential, { messageId, senderSeq: 7 });
    expect(response.status).toBe(200);
    const body = await json(response);
    expect(body.messageId).toBe(messageId);
    expect(body.senderSeq).toBe(7);
    expect(body.serverSeq).toBe(1);
    expect(typeof body.acceptedAt).toBe("number");

    const row = await env.DB.prepare(
      "SELECT sender_device_id, sender_participant, sender_seq, server_seq FROM mailbox_messages WHERE message_id = ?1",
    ).bind(messageId).first<{ sender_device_id: string; sender_participant: string; sender_seq: number; server_seq: number }>();
    ...
    expect(row?.sender_device_id).toBe(me.deviceId);
    expect(row?.sender_participant).toBe("ME");
    expect(row?.sender_seq).toBe(7);
  ...
  it("returns the original result for an exact retry without advancing state", async () => {
    const { me } = await bootstrapAndAccept();
    const messageId = crypto.randomUUID();
    const request = pushRequest(me.credential, { messageId, senderSeq: 4, ciphertext: "stable" });
    const first = await exports.default.fetch("https://rucola.test/v1/sync/push", request);
    const firstBody = await json(first);
    const second = await exports.default.fetch("https://rucola.test/v1/sync/push", pushRequest(me.credential, { messageId, senderSeq: 4, ciphertext: "stable" }));
  ...
  it("handles concurrent exact retries idempotently", async () => {
    ...
    const requests = Array.from({ length: 10 }, () =>
      exports.default.fetch("https://rucola.test/v1/sync/push", pushRequest(me.credential, { messageId, senderSeq: 9, ciphertext: "same" })),
    );
  ...
  it("rejects a conflicting payload for an existing message ID", async () => {
    const { me } = await bootstrapAndAccept();
    const messageId = crypto.randomUUID();
    expect((await push(me.credential, { messageId, senderSeq: 1, ciphertext: "original" })).status).toBe(200);
    const response = await push(me.credential, { messageId, senderSeq: 1, ciphertext: "tampered" });
    expect(response.status).toBe(409);
    expect((await json(response)).error).toMatchObject({ code: "MESSAGE_ID_CONFLICT" });
  });

  it("rejects reuse of a sender sequence for another message", async () => {
    const { me } = await bootstrapAndAccept();
    expect((await push(me.credential, { senderSeq: 12, ciphertext: "first" })).status).toBe(200);
    const response = await push(me.credential, { senderSeq: 12, ciphertext: "second" });
    expect(response.status).toBe(409);
    expect((await json(response)).error).toMatchObject({ code: "SENDER_SEQUENCE_CONFLICT" });
  });

  it("accepts gaps and out-of-order sender sequences", async () => {
    const { me } = await bootstrapAndAccept();
    const high = await push(me.credential, { senderSeq: 12, ciphertext: "twelve" });
    const low = await push(me.credential, { senderSeq: 10, ciphertext: "ten" });
    expect(high.status).toBe(200);
    expect(low.status).toBe(200);

    const rows = await env.DB.prepare(
      "SELECT sender_seq, server_seq FROM mailbox_messages WHERE relationship_id = ?1 ORDER BY server_seq",
    ).bind(me.relationshipId).all<{ sender_seq: number; server_seq: number }>();
  ...
  it("serializes concurrent distinct messages with unique server sequences", async () => {
    ...
    const responses = await Promise.all(
      Array.from({ length: 8 }, (_, index) =>
        push(me.credential, { senderSeq: index + 1, ciphertext: `cipher-${index + 1}` }),
      ),
    );
    expect(responses.every((response) => response.status === 200)).toBe(true);
  ...
  it("rejects malformed, unsupported, and oversized payloads", async () => {
    ...
    const invalidType = await push(me.credential, { type: "VIDEO" });
    expect(invalidType.status).toBe(400);

    const oversized = await push(me.credential, { ciphertext: "x".repeat(13 * 1024) });
  ...
  it("does not trust client-supplied relationship or participant fields", async () => {
    ...
    const body = {
      ...
      encryptionVersion: 1,
      createdAt: Date.now(),
      relationshipId: "attacker-controlled",
      participant: "PARTNER",
    };
    ...
    const row = await env.DB.prepare(
      "SELECT relationship_id, sender_participant FROM mailbox_messages WHERE message_id = ?1",
    ).bind(body.messageId).first<{ relationship_id: string; sender_participant: string }>();
    expect(row?.relationship_id).toBe(me.relationshipId);
    expect(row?.sender_participant).toBe("ME");
  });
  ...
  it("attaches only an owned READY media upload", async () => {
    ...
    const mailbox = await env.DB.prepare("SELECT media_upload_id FROM mailbox_messages WHERE relationship_id = ?1 AND media_upload_id = ?2").bind(me.relationshipId, mediaId).first<{ media_upload_id: string }>();
  ...
  it("rolls back a failed server-sequence allocation without consuming state", async () => {
    const { me } = await bootstrapAndAccept();
    const existing = await push(me.credential, { senderSeq: 1, ciphertext: "existing" });
    expect(existing.status).toBe(200);

    await env.DB.prepare("UPDATE relationships SET next_server_seq = 1 WHERE id = ?1").bind(me.relationshipId).run();
    const failed = await push(me.credential, { senderSeq: 2, ciphertext: "should-not-stick" });
