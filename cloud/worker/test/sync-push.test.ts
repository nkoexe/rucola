...
async function insertReadyMedia(
  relationshipId: string,
  deviceId: string,
  mediaId: string,
): Promise<void> {
  ...
  await env.DB.prepare(
    `INSERT INTO media_uploads (id, relationship_id, created_by_device_id, object_key, media_type, declared_mime, size_bytes, status, created_at, expires_at, completed_at)
     VALUES (?1, ?2, ?3, ?4, 'PHOTO', 'image/jpeg', 10, 'READY', ?5, ?6, ?5)`,
  )
    .bind(mediaId, relationshipId, deviceId, `test/${mediaId}`, now, now + 3600000)
    .run();
}
...
describe("Rucola mailbox push", () => {
  it("accepts a first message and allocates serverSeq exactly once", async () => {
    ...
    const row = await env.DB
      .prepare("SELECT sender_device_id, sender_participant, sender_seq, server_seq FROM mailbox_messages WHERE message_id = ?1")
      .bind(messageId)
      .first<{ sender_device_id: string; sender_participant: string; sender_seq: number; server_seq: number }>();
  ...
  it("returns the original result for an exact retry without advancing state", async () => {
    ...
    const first = await exports.default.fetch(
      "https://rucola.test/v1/sync/push",
      pushRequest(me.credential, body),
    );
  ...
  it("rejects a conflicting payload for an existing message ID", async () => {
    ...
    const response = await push(me.credential, {
      messageId,
      senderSeq: 1,
      ciphertext: "tampered",
    });
  ...
  it("accepts gaps and out-of-order sender sequences", async () => {
    ...
    const rows = await env.DB
      .prepare("SELECT sender_seq, server_seq FROM mailbox_messages WHERE relationship_id = ?1 ORDER BY server_seq")
      .bind(me.relationshipId)
      .all<{ sender_seq: number; server_seq: number }>();
    expect(rows.results.map((row) => [row.sender_seq, row.server_seq])).toEqual([
      [12, 1],
      [10, 2],
    ]);
  });
  ...
  it("serializes concurrent distinct messages with unique server sequences", async () => {
    ...
    expect(responses.every((response) => response.status === 200)).toBe(true);
    const rows = await env.DB
      .prepare("SELECT server_seq FROM mailbox_messages WHERE relationship_id = ?1 ORDER BY server_seq")
      .bind(me.relationshipId)
  ...
  it("rejects malformed, unsupported, and oversized payloads", async () => {
    ...
    expect((await push(me.credential, { type: "VIDEO" })).status).toBe(400);
    expect((await push(me.credential, { ciphertext: "x".repeat(13 * 1024) })).status).toBe(413);
  });
  ...
  it("does not trust client-supplied relationship or participant fields", async () => {
    ...
    expect(response.status).toBe(200);
    const row = await env.DB
      .prepare("SELECT relationship_id, sender_participant FROM mailbox_messages WHERE message_id = ?1")
      .bind(body.messageId)
      .first<{ relationship_id: string; sender_participant: string }>();
  ...
  it("attaches only an owned READY media upload", async () => {
    ...
    await insertReadyMedia(me.relationshipId, me.deviceId, mediaId);
    ...
    const response = await push(me.credential, {
      ...
      ciphertext: "photo-meta",
      createdAt,
    });
    expect(response.status).toBe(200);

    const media = await env.DB
      .prepare("SELECT status FROM media_uploads WHERE id = ?1")
      .bind(mediaId)
      .first<{ status: string }>();
    const mailbox = await env.DB
      .prepare("SELECT media_upload_id FROM mailbox_messages WHERE relationship_id = ?1 AND media_upload_id = ?2")
      .bind(me.relationshipId, mediaId)
      .first<{ media_upload_id: string }>();
    expect(media?.status).toBe("ATTACHED");
    expect(mailbox?.media_upload_id).toBe(mediaId);

    const retry = await push(me.credential, {
      messageId,
      senderSeq: 1,
      type: "PHOTO_VIDEO",
      mediaUploadId: mediaId,
  ...
  it("does not allow one media upload to be attached to two messages concurrently", async () => {
    ...
    const mailbox = await env.DB
      .prepare("SELECT COUNT(*) AS count FROM mailbox_messages WHERE relationship_id = ?1 AND media_upload_id = ?2")
      .bind(me.relationshipId, mediaId)
      .first<{ count: number }>();
    const media = await env.DB
      .prepare("SELECT status FROM media_uploads WHERE id = ?1")
  ...
  it("rolls back a failed server-sequence allocation without consuming state", async () => {
    const { me } = await bootstrapAndAccept();
    expect((await push(me.credential, { senderSeq: 1, ciphertext: "existing" })).status).toBe(200);
    await env.DB
      .prepare("UPDATE relationships SET next_server_seq = 1 WHERE id = ?1")
      .bind(me.relationshipId)
      .run();
    const failed = await push(me.credential, {
      senderSeq: 2,
      ciphertext: "should-not-stick",
    });
