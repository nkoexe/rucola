...
async function bootstrapAndAccept(): Promise<{ me: BootstrapBody; partner: AcceptedBody }> {
  const bootstrapResponse = await exports.default.fetch("https://rucola.test/v1/pairing/bootstrap", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ expiresInSeconds: 3600 }) });
...
async function insertReadyMedia(relationshipId: string, deviceId: string, mediaId: string): Promise<void> {
  ...
  ).bind(mediaId, relationshipId, deviceId, `test/${mediaId}`, now, now + 3600000).run();
}
...
describe("Rucola mailbox push", () => {
  ...
  it("handles concurrent exact retries idempotently", async () => {
    const { me } = await bootstrapAndAccept(); const messageId = crypto.randomUUID();
    const responses = await Promise.all(Array.from({ length: 10 }, () => exports.default.fetch("https://rucola.test/v1/sync/push", pushRequest(me.credential, { messageId, senderSeq: 9, ciphertext: "same" }))));
  ...
  it("rejects reuse of a sender sequence for another message", async () => {
    const { me } = await bootstrapAndAccept(); expect((await push(me.credential, { senderSeq: 12, ciphertext: "first" })).status).toBe(200);
    const response = await push(me.credential, { senderSeq: 12, ciphertext: "second" }); expect(response.status).toBe(409); expect((await json(response)).error).toMatchObject({ code: "SENDER_SEQUENCE_CONFLICT" });
  });

  it("accepts gaps and out-of-order sender sequences", async () => {
    const { me } = await bootstrapAndAccept(); const high = await push(me.credential, { senderSeq: 12, ciphertext: "twelve" }); const low = await push(me.credential, { senderSeq: 10, ciphertext: "ten" });
    expect(high.status).toBe(200); expect(low.status).toBe(200);
    const rows = await env.DB.prepare("SELECT sender_seq, server_seq FROM mailbox_messages WHERE relationship_id = ?1 ORDER BY server_seq").bind(me.relationshipId).all<{ sender_seq: number; server_seq: number }>();
    expect(rows.results.map((row) => [row.sender_seq, row.server_seq])).toEqual([[12, 1], [10, 2]]);
  });
  ...
  it("serializes concurrent distinct messages with unique server sequences", async () => {
    ...
    expect(rows.results.map((row) => row.server_seq)).toEqual([1, 2, 3, 4, 5, 6, 7, 8]);
  });

  it("rejects pushes from revoked devices", async () => {
    const { me } = await bootstrapAndAccept(); await env.DB.prepare("UPDATE devices SET revoked_at = ?1 WHERE id = ?2").bind(Date.now(), me.deviceId).run();
    expect((await push(me.credential, { senderSeq: 1 })).status).toBe(401);
  });

  it("rejects pushes while the relationship is still pairing", async () => {
    const bootstrapResponse = await exports.default.fetch("https://rucola.test/v1/pairing/bootstrap", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ expiresInSeconds: 3600 }) });
    const me = (await json(bootstrapResponse)) as unknown as BootstrapBody; const response = await push(me.credential, { senderSeq: 1 });
    expect(response.status).toBe(409); expect((await json(response)).error).toMatchObject({ code: "RELATIONSHIP_INACTIVE" });
  });

  it("rejects pushes after a relationship ends", async () => {
    const { me } = await bootstrapAndAccept(); await env.DB.prepare("UPDATE relationships SET status = 'ENDED', ended_at = ?1 WHERE id = ?2").bind(Date.now(), me.relationshipId).run();
    const response = await push(me.credential, { senderSeq: 1 }); expect(response.status).toBe(409); expect((await json(response)).error).toMatchObject({ code: "RELATIONSHIP_INACTIVE" });
  });

  it("rejects malformed, unsupported, and oversized payloads", async () => {
    const { me } = await bootstrapAndAccept();
    const malformed = await exports.default.fetch("https://rucola.test/v1/sync/push", { method: "POST", headers: { authorization: `Bearer ${me.credential}`, "content-type": "application/json" }, body: "null" });
  ...
  it("does not trust client-supplied relationship or participant fields", async () => {
    ...
    const response = await exports.default.fetch("https://rucola.test/v1/sync/push", { method: "POST", headers: { authorization: `Bearer ${me.credential}`, "content-type": "application/json" }, body: JSON.stringify(body) });
    expect(response.status).toBe(200); const row = await env.DB.prepare("SELECT relationship_id, sender_participant FROM mailbox_messages WHERE message_id = ?1").bind(body.messageId).first<{ relationship_id: string; sender_participant: string }>();
    expect(row?.relationship_id).toBe(me.relationshipId); expect(row?.sender_participant).toBe("ME");
  });

  it("attaches only an owned READY media upload", async () => {
    const { me, partner } = await bootstrapAndAccept(); const mediaId = crypto.randomUUID(); await insertReadyMedia(me.relationshipId, me.deviceId, mediaId);
    const response = await push(me.credential, { type: "PHOTO_VIDEO", mediaUploadId: mediaId, ciphertext: "photo-meta" }); expect(response.status).toBe(200);
    const media = await env.DB.prepare("SELECT status FROM media_uploads WHERE id = ?1").bind(mediaId).first<{ status: string }>(); const mailbox = await env.DB.prepare("SELECT media_upload_id FROM mailbox_messages WHERE relationship_id = ?1 AND media_upload_id = ?2").bind(me.relationshipId, mediaId).first<{ media_upload_id: string }>();
    expect(media?.status).toBe("ATTACHED"); expect(mailbox?.media_upload_id).toBe(mediaId);
    const retry = await push(me.credential, { type: "PHOTO_VIDEO", mediaUploadId: mediaId, ciphertext: "photo-meta" }); expect(retry.status).toBe(200);
    const otherMediaId = crypto.randomUUID(); await insertReadyMedia(me.relationshipId, partner.deviceId, otherMediaId);
    const ownership = await push(me.credential, { type: "PHOTO_VIDEO", mediaUploadId: otherMediaId, ciphertext: "other-photo" }); expect(ownership.status).toBe(409); expect((await json(ownership)).error).toMatchObject({ code: "MEDIA_CONFLICT" });
  });

  it("does not allow one media upload to be attached to two messages concurrently", async () => {
    const { me } = await bootstrapAndAccept(); const mediaId = crypto.randomUUID(); await insertReadyMedia(me.relationshipId, me.deviceId, mediaId);
    ...
    expect(responses.filter((response) => response.status === 200)).toHaveLength(1); expect(responses.filter((response) => response.status === 409)).toHaveLength(1);
    const mailbox = await env.DB.prepare("SELECT COUNT(*) AS count FROM mailbox_messages WHERE relationship_id = ?1 AND media_upload_id = ?2").bind(me.relationshipId, mediaId).first<{ count: number }>(); const media = await env.DB.prepare("SELECT status FROM media_uploads WHERE id = ?1").bind(mediaId).first<{ status: string }>();
    expect(mailbox?.count).toBe(1); expect(media?.status).toBe("ATTACHED");
  });

  it("rolls back a failed server-sequence allocation without consuming state", async () => {
    const { me } = await bootstrapAndAccept(); expect((await push(me.credential, { senderSeq: 1, ciphertext: "existing" })).status).toBe(200);
    await env.DB.prepare("UPDATE relationships SET next_server_seq = 1 WHERE id = ?1").bind(me.relationshipId).run(); const failed = await push(me.credential, { senderSeq: 2, ciphertext: "should-not-stick" });
    expect(failed.status).toBe(500); const count = await env.DB.prepare("SELECT COUNT(*) AS count FROM mailbox_messages WHERE relationship_id = ?1").bind(me.relationshipId).first<{ count: number }>(); const relationship = await env.DB.prepare("SELECT next_server_seq FROM relationships WHERE id = ?1").bind(me.relationshipId).first<{ next_server_seq: number }>();
