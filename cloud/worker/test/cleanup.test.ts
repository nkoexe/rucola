...
async function bootstrapAndAccept(): Promise<{ me: PairingBody; partner: PairingBody }> {
  ...
  const bootstrapResponse = await exports.default.fetch("https://rucola.test/v1/pairing/bootstrap", {
    ...
    headers: {
      ...
      "cf-connecting-ip": `198.51.100.${((testId - 1) % 254) + 1}`,
    },
...
async function uploadAndComplete(credential: string, uploadId: string): Promise<void> {
  const uploadResponse = await exports.default.fetch(`https://rucola.test/v1/media/${uploadId}`, {
    method: "PUT",
    headers: {
      authorization: `Bearer ${credential}`,
  ...
  const completeResponse = await exports.default.fetch(`https://rucola.test/v1/media/${uploadId}/complete`, {
    method: "POST",
...
describe("cleanup lifecycle", () => {
  ...
  it("keeps attached media while its durable receipt is retained", async () => {
    ...
    await env.DB.prepare(
      "UPDATE media_uploads SET status = 'ATTACHED', attached_at = ? WHERE id = ?",
    ).bind(now, uploadId).run();
    await env.DB.prepare(
      `INSERT INTO message_receipts
        (relationship_id, message_id, sender_device_id, sender_seq, type, ciphertext_hash,
         encryption_version, client_created_at, media_upload_id, server_seq, server_received_at,
         created_at, delivery_expires_at, retention_expires_at, acknowledged_at)
       VALUES (?, ?, ?, 1, 'PHOTO_VIDEO', ?, 1, ?, ?, 1, ?, ?, ?, ?, ?)`
    ).bind(
      me.relationshipId,
      `cleanup-${testId}`,
      me.deviceId,
    ...
    const result = await runCleanup(env, now + 2000);

    expect(result.expiredReceipts).toBe(0);
    expect(await env.DB.prepare("SELECT status FROM media_uploads WHERE id = ?").bind(uploadId).first<{ status: string }>()).toEqual({ status: "ATTACHED" });
    expect(await env.DB.prepare("SELECT message_id FROM message_receipts WHERE media_upload_id = ?").bind(uploadId).first()).not.toBeNull();
  });
  ...
  it("releases attached media after receipt retention expires", async () => {
    ...
    const messageId = `cleanup-retained-${testId}`;
    await env.DB.prepare(
      `INSERT INTO message_receipts
        (relationship_id, message_id, sender_device_id, sender_seq, type, ciphertext_hash,
         encryption_version, client_created_at, media_upload_id, server_seq, server_received_at,
