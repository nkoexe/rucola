...
interface MediaUploadRow {
  id: string; relationship_id: string; created_by_device_id: string; object_key: string;
  media_type: ReservationType; declared_mime: string; size_bytes: number; checksum: string | null;
  status: MediaStatus; expires_at: number;
}
...
async function readJson(request: Request): Promise<CreateMediaRequest | null> {
  ...
  try {
    while (true) {
      ...
      if (totalBytes > MAX_JSON_BODY_BYTES) { await reader.cancel(); return null; }
      chunks.push(value);
    }
...
function parseMime(value: unknown): string | null {
  if (typeof value !== "string" || value.length === 0 || utf8ByteLength(value) > MAX_MIME_BYTES) return null;
  return value.trim().toLowerCase() || null;
}
function parseSize(value: unknown): number | null {
  if (!Number.isSafeInteger(value)) return null;
  const size = value as number;
  return size >= MIN_MEDIA_BYTES ? size : null;
}
...
async function getOwnedUpload(env: Env, device: AuthenticatedDevice, uploadId: string): Promise<MediaUploadRow | null> {
  return env.DB.prepare(
    `SELECT media_uploads.id, media_uploads.relationship_id, media_uploads.created_by_device_id,
            media_uploads.object_key, media_uploads.media_type, media_uploads.declared_mime,
            media_uploads.size_bytes, media_uploads.checksum, media_uploads.status, media_uploads.expires_at
     FROM media_uploads
...
export async function createMediaReservation(env: Env, request: Request): Promise<Response> {
  ...
  if (!mime || !mimeAllowed(type, mime)) return errorResponse("UNSUPPORTED_MEDIA_TYPE", "MIME type is not supported for this media type", 400);
  ...
  try {
    const result = await env.DB.prepare(
      `INSERT INTO media_uploads (
         ...
         size_bytes, checksum, status, created_at, expires_at, completed_at, attached_at
       )
       SELECT ?1, ?2, ?3, ?4, ?5, ?6, ?7, ?8, 'PENDING', ?9, ?10, NULL, NULL
       FROM relationships WHERE id = ?2 AND status = 'ACTIVE'`,
  ...
  } catch { return errorResponse("DATABASE_UNAVAILABLE", "Media reservation could not be created", 503); }
...
export async function uploadMedia(env: Env, request: Request, uploadId: string): Promise<Response> {
  ...
  if (!Number.isSafeInteger(contentLength) || contentLength !== upload.size_bytes) return errorResponse("MEDIA_SIZE_MISMATCH", "Content-Length does not match the reservation", 400);
  if (!request.body) return errorResponse("INVALID_REQUEST", "Media request body required", 400);
  let bytesSeen = 0;
  const boundedStream = request.body.pipeThrough(new TransformStream<Uint8Array, Uint8Array>({
    transform(chunk, controller) {
      bytesSeen += chunk.byteLength;
      if (bytesSeen > upload.size_bytes) { controller.error(new Error("media body exceeds reservation")); return; }
...
export async function completeMedia(env: Env, request: Request, uploadId: string): Promise<Response> {
  ...
  try { object = await env.MEDIA_BUCKET.head(upload.object_key); }
  catch { return errorResponse("MEDIA_STORAGE_UNAVAILABLE", "Media storage could not be checked", 502); }
  if (!object) return errorResponse("MEDIA_NOT_UPLOADED", "Media object has not been uploaded", 409);
  if (object.size !== upload.size_bytes || object.httpMetadata?.contentType !== upload.declared_mime || object.customMetadata?.uploadId !== upload.id || (upload.checksum !== null && !sameSha256(object, upload.checksum))) {
    await removeObjectBestEffort(env, upload.object_key);
  ...
  try {
    const result = await env.DB.prepare(
      `UPDATE media_uploads SET status = 'READY', completed_at = ?1, expires_at = ?2
       WHERE id = ?3 AND relationship_id = ?4 AND created_by_device_id = ?5 AND status = 'PENDING' AND expires_at > ?1`,
  ...
  } catch { return errorResponse("DATABASE_UNAVAILABLE", "Media completion could not be recorded", 503); }
  return json({ uploadId: upload.id, status: "READY" });
}
