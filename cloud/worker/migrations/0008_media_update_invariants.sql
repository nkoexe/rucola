-- Media reservation constraints must also hold if lifecycle metadata is
-- ever corrected or migrated through an UPDATE.

CREATE TRIGGER media_upload_size_update_invariant
BEFORE UPDATE OF media_type, size_bytes ON media_uploads
BEGIN
  SELECT CASE
...
CREATE TRIGGER media_upload_mime_update_invariant
BEFORE UPDATE OF media_type, declared_mime ON media_uploads
BEGIN
  SELECT CASE
