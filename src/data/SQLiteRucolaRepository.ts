...
export class SQLiteRucolaRepository implements RucolaRepository {
  ...
  async getRelationship(): Promise<Relationship | null> {
    const row = await this.db.getFirstAsync<RelationshipRow>('SELECT * FROM relationships WHERE id = ?', RELATIONSHIP_ID);
  ...
  async getMessages(): Promise<Message[]> {
    const rows = await this.db.getAllAsync<MessageRow>(
      `SELECT m.*, CASE WHEN s.messageId IS NULL THEN 0 ELSE 1 END AS isActive
       FROM messages m LEFT JOIN active_message_slots s ON s.messageId = m.id
       WHERE m.relationshipId = ? ORDER BY m.orderIndex DESC, m.createdAt DESC, m.id DESC`,
      RELATIONSHIP_ID,
    );
  ...
  async getActiveMessage(participant: Participant): Promise<Message | null> {
    const row = await this.db.getFirstAsync<MessageRow>(
      `SELECT m.* FROM messages m INNER JOIN active_message_slots s ON s.messageId = m.id
       WHERE s.relationshipId = ? AND s.participant = ?`, RELATIONSHIP_ID, participant,
    );
