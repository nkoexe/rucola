...
export async function runSyncStateIntegrationTests(db: SQLite.SQLiteDatabase): Promise<void> {
  ...
  const blocked = await db.getFirstAsync<{ blocked: number }>(
    'SELECT blocked FROM sync_outbox WHERE relationshipId = ? AND messageId = ?',
    'the-one',
    first.id,
  );
  ...
  await assertRejects(
    () => store.commitInbound([{ id: 'remote-old', type: 'TEXT', body: 'too old', createdAt: Date.UTC(2019, 0, 1), serverSeq: 1 }], 1),
    'Inbound timestamps outside the supported range must be rejected',
  );
  await assertRejects(
    () => store.commitInbound([{ id: 'remote-future', type: 'TEXT', body: 'too future', createdAt: Date.now() + 8 * 24 * 60 * 60 * 1000, serverSeq: 1 }], 1),
    'Inbound timestamps too far in the future must be rejected',
  );
  await assertRejects(
    () => store.commitInbound([{ id: 'remote-emoji', type: 'EMOJI', body: '   ', createdAt, serverSeq: 1 }], 1),
    'Whitespace-only emoji messages must be rejected',
  );
  await assertRejects(
    () => store.commitInbound([{ id: 'remote-drawing', type: 'DRAWING', body: '', createdAt, serverSeq: 1 }], 1),
    'Drawing messages without media must be rejected',
  );
  await assertRejects(
    () => store.commitInbound([{ id: 'remote-cursor', type: 'TEXT', body: 'cursor mismatch', createdAt, serverSeq: 1 }], 2),
    'Non-empty inbound batches must end at nextCursor',
  );
  ...
  await assertRejects(
    () => store.commitInbound([{ id: 'remote-1', type: 'TEXT', body: 'tampered', createdAt, serverSeq: 1 }], 1),
  ...
  await assertRejects(
    () => store.commitInbound([{ id: 'remote-1', type: 'TEXT', body: 'hello', createdAt, serverSeq: 1 }], 1, createdAt + 4_000),
    'Inbound cursor must never move backwards',
  );

  await db.runAsync(
    "INSERT INTO messages (id, relationshipId, participant, type, body, createdAt, orderIndex, mediaReference, syncState) VALUES (?, ?, 'PARTNER', 'TEXT', ?, ?, (SELECT COALESCE(MAX(orderIndex), 0) + 1 FROM messages WHERE relationshipId = ?), NULL, 'SYNCED')",
    'legacy-remote',
    'the-one',
    'legacy replay',
    createdAt + 5,
  ...
  const legacyReceipt = await db.getFirstAsync<{ serverSeq: number }>(
