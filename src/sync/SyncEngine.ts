...
export class SyncEngine {
  ...
  run(): Promise<SyncRunResult> {
    if (!this.runPromise) this.runPromise = this.runInternal().finally(() => { this.runPromise = null; });
  ...
  private async pullIncoming(result: SyncRunResult): Promise<void> {
    ...
    if (cursor > 0) {
      ...
      if (pendingAck.acknowledgedThrough < cursor) {
        throw new Error('Cloud acknowledged fewer messages than the persisted local cursor.');
      }
    ...
    while (hasMore) {
      ...
      for (const remote of response.messages) {
        ...
        if (!isValidDeviceId(remote.senderDeviceId) || remote.senderParticipant !== 'PARTNER') {
      ...
      if (response.nextCursor !== lastServerSeq || response.nextCursor <= cursor) {
        throw new Error('Cloud returned a non-advancing or inconsistent pull cursor.');
      }
