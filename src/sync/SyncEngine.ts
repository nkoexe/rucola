...
export interface SyncCodec {
  ...
  decrypt(message: CloudPulledMessage): Promise<{ type: CloudMessageType; body: string; mediaReference?: string | null }>;
}
...
function isRetryableSyncError(cause: unknown): boolean { if (!(cause instanceof CloudClientError)) return false; return cause.status === 0 || cause.status === 408 || cause.status === 429 || cause.status >= 500; }
...
export class SyncEngine {
  ...
  constructor(options: SyncEngineOptions) {
    this.cloud = options.cloud; this.state = options.state; this.repository = options.repository; this.codec = options.codec;
    this.now = options.now ?? Date.now; this.outboxBatchSize = normalizeBatchSize(options.outboxBatchSize, DEFAULT_OUTBOX_BATCH_SIZE); this.pullBatchSize = normalizeBatchSize(options.pullBatchSize, DEFAULT_PULL_BATCH_SIZE);
  ...
  private async runInternal(): Promise<SyncRunResult> {
    ...
    await this.pushDue(result); await this.pullIncoming(result); return result;
  }

  private async pushDue(result: SyncRunResult): Promise<void> {
    await this.state.reconcileOutbox(this.now());
    const due = await this.state.getDueOutbox(this.now(), this.outboxBatchSize); if (due.length === 0) return;
  ...
  private async pullIncoming(result: SyncRunResult): Promise<void> {
    let cursor = await this.state.getPullCursor(); let hasMore = true;
    while (hasMore) {
      ...
      if (response.messages.length === 0) {
        ...
        result.moreIncoming = false; return;
      }
      const inbound: InboundSyncMessage[] = []; let previousServerSeq = cursor;
      ...
      cursor = response.nextCursor; result.pulled += inbound.length; result.acknowledged += ack.deleted; hasMore = response.hasMore; result.moreIncoming = hasMore;
    }
  }
}
