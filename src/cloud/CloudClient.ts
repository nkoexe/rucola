...
export class CloudClient {
  ...
  async joinPairingSession(confirmationCode: string, share: string): Promise<PairingSessionJoinResponse> {
    return this.request<PairingSessionJoinResponse>({
      method: 'POST',
      path: '/v1/pairing/session',
      body: { action: 'JOIN', confirmationCode, share },
